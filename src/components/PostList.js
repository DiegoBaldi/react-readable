        ...
        >
          {posts.sort(sortBy(orderByObj.command)).map(post =>
            <Box w={[1, 1 / 2, 1 / 3, 1 / 4]} key={post.id}>
              <ReadableCard element={post} history={history} cardType="post" />
            </Box>
          )}
        </div>
        <button
  ...
  return {
    posts: Object.keys(posts)
      ...
      .filter(
        post =>
          !post.deleted &&
          (ownProps.match && ownProps.match.params.category
            ? post.category === categories[ownProps.match.params.category].name
            : true)
      ),
...
export default connect(mapStateToProps, {
  ...
  addPost,
  editPost,
  cyclePostOrderBy: cyclePostsOrderBySettings
})(PostList)
